...
def setup_page():
    ...
    st.set_page_config(page_title="Face Cut App", page_icon="✂️")
...
def display_ui():
    ...
    st.sidebar.markdown("## Image Face")
...
def process_and_display_images(uploaded_files):
    ...
    if not uploaded_files:
        st.info('Please upload an image.', icon="ℹ️")
        return

    if not st.sidebar.button("Extract Face"):
        return
    ...
    with st.spinner("Removing backgrounds..."):
        for uploaded_file in uploaded_files:
            ...
            result_image = process_face_cut(uploaded_file.getvalue())
...
def process_face_cut(image_bytes):
    ...
    if len(faces) >= 2 :
        ...
        crop_face = face_model.crop_face(img)
        crop_face = Image.fromarray(crop_face)
        result = model.infer(img=crop_face)
        result = img_to_bytes(result)
        st.success('Face cut done successfully', icon="✅")
        return Image.open(io.BytesIO(result)).convert("RGBA")
